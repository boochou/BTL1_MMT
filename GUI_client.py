...
class Client:
    ...
    def connect_server(self,IP):
        try:
            ...
            self.thread_ping = threading.Thread(target=self.handle_listen)
            self.thread_ping.daemon = False
    ...
    def fetch_GUI(self, fname):
        ...
        try:
            ...
            return "3"
    def fetch_GUI_P2P(self,fname,addr):
    ...
    def delete_file(self, fname, fpath):
        self.socket.send("DELETE".encode(FORMAT))
        try: 
            mess_from_server = self.socket.recv(1024).decode(FORMAT)
            if mess_from_server == "File name to delete?":
                fname_username = (fname, self.username)
                self.socket.send(json.dumps(fname_username).encode(FORMAT))
            ...
            try:
                os.remove(file_path)
                print(f"File '{fname}' deleted locally.")
                # Notify the server that the local deletion was successful
                self.socket.send("DELETE_SUCCESS".encode(FORMAT))
                mess_from_server = self.socket.recv(1024).decode(FORMAT)
                print(str(mess_from_server))
            except Exception as e:
                print(f"Failed to delete file '{fname}' locally: {e}")
                # Notify the server that the local deletion failed
                self.socket.send("DELETE_FAIL".encode(FORMAT))
                return "3"
        except Exception as e:
            print(f"Failed to delete file '{fname}': {e}")
            return "3"
